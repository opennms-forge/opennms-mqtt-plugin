...
public class Controller {
	...
	private Map<String, MQTTClientImpl> m_clientMap = new ConcurrentHashMap<String, MQTTClientImpl>();

	// internal map of client name / mqttrx service
	private Map<String, MqttRxService> m_rxServiceMap = new ConcurrentHashMap<String, MqttRxService>();
	...
	private String m_configDirectory = null;
	...
	public void setNodeByForeignSourceCacheImpl(NodeByForeignSourceCacheImpl nodeByForeignSourceCacheImpl) {
	...
	/**
	 * called by init to load configuration and set up controlled classes before
	 * start();
	 */
	public void loadConfig() {

		m_MQTTReceiverConfig = loadConfigFiles();

		// set up node cache
		if (m_nodeByForeignSourceCacheImpl == null)
			throw new IllegalStateException("nodeByForeignSourceCacheImpl must be set");
		if (m_MQTTReceiverConfig.getNodeCacheMaxSize() != null)
			m_nodeByForeignSourceCacheImpl.setMAX_SIZE(m_MQTTReceiverConfig.getNodeCacheMaxSize());
		if (m_MQTTReceiverConfig.getNodeCacheMaxTtl() != null)
			m_nodeByForeignSourceCacheImpl.setMAX_TTL(m_MQTTReceiverConfig.getNodeCacheMaxTtl());
		if (m_MQTTReceiverConfig.getCreateDummyInterfaces() != null)
			m_nodeByForeignSourceCacheImpl.setCreateDummyInterfaces(m_MQTTReceiverConfig.getCreateDummyInterfaces());
		if (m_MQTTReceiverConfig.getCreateMissingNodes() != null)
			m_nodeByForeignSourceCacheImpl.setCreateMissingNodes(m_MQTTReceiverConfig.getCreateMissingNodes());
		if (m_MQTTReceiverConfig.getCreateNodeAssetData() != null)
			m_nodeByForeignSourceCacheImpl.setcreateNodeAssetData(m_MQTTReceiverConfig.getCreateNodeAssetData());
		...
		for (MQTTClientConfigXml mqttConfigXml : m_MQTTReceiverConfig.getMqttClients()) {
			...
			if (m_clientMap.containsKey(mqttConfigXml.getClientInstanceId()))
				throw new IllegalArgumentException("duplicate mqtt receiver ClientInstanceId '"
						+ mqttConfigXml.getClientInstanceId() + "' in configuration");
		...
		// m_rxServiceMap will contain a list of all the message clients defined in the
		// config
			...
			for (MqttRxService messageReceiver : m_messageReceiverServices) {
				...
				if (messageReceiver.getClientInstanceId() == null || "".equals(messageReceiver.getClientInstanceId()))
					throw new IllegalArgumentException(
							"clientInstanceId value is not defined for a Receiver in blueprint");

				if (messageReceivers.containsKey(messageReceiver.getClientInstanceId()))
					throw new IllegalArgumentException("duplicate messageReceiver ClientInstanceId in blueprint '"
							+ messageReceiver.getClientInstanceId() + "' in configuration");
				messageReceivers.put(messageReceiver.getClientInstanceId(), messageReceiver);
			}
		}

		// find the client in the blueprint corresponding to the configuration file and
		// configure it
		for (MessageClientConfig receiverConfig : m_MQTTReceiverConfig.getMessageClients()) {
			...
			if (receiverConfig.getClientInstanceId() == null || "".equals(receiverConfig.getClientInstanceId()))
				throw new IllegalArgumentException(
						"clientInstanceId value is not defined for a Receiver in configuration");
			...
			if (rxService != null && rxService.getClientType().equals(receiverConfig.getClientType())) {
				...
				if (receiverConfig.getTopicList() != null)
					rxService.setTopicList(receiverConfig.getTopicList());
				...
				throw new IllegalArgumentException("receiver configId " + receiverConfig.getClientInstanceId()
						+ " clientType " + receiverConfig.getClientType() + " defines an unavailable service");
			}
		}

		// set up message queue
		if (m_messageNotificationClientQueueImpl == null)
			throw new IllegalStateException("m_messageNotificationClientQueueImpl has not been defined");
		if(m_MQTTReceiverConfig.getMaxMessageQueueLength()==null) 
			throw new IllegalStateException("m_MQTTReceiverConfig.getMaxMessageQueueLength has not been defined");
		if(m_MQTTReceiverConfig.getMaxMessageQueueThreads()==null) 
			throw new IllegalStateException("m_MQTTReceiverConfig.getMaxMessageQueueThreads() has not been defined");
		...
		// NOTE notification handing clients are set up directly in the blueprint and
		// not here
		// List<NotificationClient> notificationHandlingClients =
		// Arrays.asList(m_notificationMessageHandler);
		...
		for (MessageDataParserConfig messageDataParserConfig : m_MQTTReceiverConfig.getMessageDataParsers()) {
			for (String subscription : messageDataParserConfig.getSubscriptionTopics()) {
				if (topicDataParserMap.containsKey(subscription))
					throw new IllegalArgumentException(
							"duplicate data topic subscription '" + subscription + "' in configuration");
		...
		for (MessageEventParserConfig messageEventParserConfig : m_MQTTReceiverConfig.getMessageEventParsers()) {
			for (String subscription : messageEventParserConfig.getSubscriptionTopics()) {
				if (topicEventParserMap.containsKey(subscription))
					throw new IllegalArgumentException(
							"duplicate event topic subscription '" + subscription + "' in configuration");
	...
	/**
	 * init method called by blueprint. Loads the configuration and then calls
	 * start()
	 */
	public void init() {
	...
	public void start() {
		...
		if (!configLoaded.get())
			throw new IllegalStateException("loadConfig() must be called and succeed before start()");
	...
	public MQTTReceiverConfig loadConfigFiles() {

		if (m_configFile == null)
			throw new RuntimeException("MQTTReceiverConfig fileUri must be set");
		if (m_configDirectory == null)
			throw new RuntimeException("MQTTReceiverConfig config directory must be set");
		...
		try {
			...
			if (mqttReceiverConfigFile.exists()) {
				...
				LOG.warn("MQTT Receiver Config successfully loaded from file="
						+ mqttReceiverConfigFile.getAbsolutePath());

				// loading additional configurations
				File mqttReceiverConfigDirectory = new File(m_configDirectory);
				if (mqttReceiverConfigDirectory.exists() && mqttReceiverConfigDirectory.isDirectory()) {
					File[] configFiles = mqttReceiverConfigDirectory.listFiles();
					for (File configFile : configFiles) {
						if (configFile.getName().endsWith(".xml")) {
							LOG.warn("loading additional configuration from file: " + configFile.getAbsolutePath());
							MQTTReceiverConfig extraConfig = (MQTTReceiverConfig) jaxbUnmarshaller
									.unmarshal(configFile);
							// check no common option set in extra config
							if (extraConfig.getNodeCacheMaxTtl() != null)
								throw new IllegalArgumentException("extra configuration should not override maxTTl");
							if (extraConfig.getCreateDummyInterfaces() != null)
								throw new IllegalArgumentException(
										"extra configuration should not override create dummy interfaces");
							if (extraConfig.getCreateMissingNodes() != null)
								throw new IllegalArgumentException(
										"extra configuration should not override create missingnodes");
							if (extraConfig.getMaxMessageQueueLength() != null)
								throw new IllegalArgumentException(
										"extra configuration should not override max message queue length");
							if (extraConfig.getMaxMessageQueueThreads() != null)
								throw new IllegalArgumentException(
										"extra configuration should not override maxx message queue threads");
							if (extraConfig.getNodeCacheMaxSize() != null)
								throw new IllegalArgumentException(
										"extra configuration should not override node cache size");

							// add configuration options
							// duplicates will be detected by controller
							Set<MessageDataParserConfig> messageDataParsers = extraConfig.getMessageDataParsers();
							if(messageDataParsers!=null) receiverConfig.getMessageDataParsers().addAll(messageDataParsers);

							Set<MessageEventParserConfig> messageEventParsers = extraConfig.getMessageEventParsers();
							if(messageEventParsers!=null) receiverConfig.getMessageEventParsers().addAll(messageEventParsers);

							Set<MQTTClientConfigXml> mqttClients = extraConfig.getMqttClients();
							if(mqttClients!=null) receiverConfig.getMqttClients().addAll(mqttClients);

							Set<MessageClientConfig> messageClients = extraConfig.getMessageClients();
							if(messageClients!=null) receiverConfig.getMessageClients().addAll(messageClients);

							LOG.warn("additional configuration successfully parsed from file: "
